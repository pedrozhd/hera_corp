...
const ListaMedicos = () => {
  ...
  useEffect(() => {
    const carregarMedicos = async () => {
      try {
        ...
        const dados = await api.medicos.listar();
  ...
  const handleEditar = (id: number) => navigate(`/medico/atualizar/${id}`);
  const handleCadastrar = () => navigate("/medico/cadastrar");
  const handleExcluir = (id: number) => navigate(`/medico/excluir/${id}`);
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    {["Nome", "CRM", "Especialidade", "E-mail", "Telefone", "Status", "Ações"].map(
                      (header) => (
                        <th
                          key={header}
                          scope="col"
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {medicos.map((m) => (
                    <tr key={m.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {m.telefone
                          ? `(${m.telefone.ddd}) ${m.telefone.numero} - ${m.telefone.tipoDeTelefone}`
                          : "—"}
                      </td>
