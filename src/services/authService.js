...
const VERIFY_ENDPOINT = `${API_BASE_URL}/auth/verify-domain`;
const AUTH_USER_STORAGE_KEY = 'auth_user';

/**
 * Verifica domínio do email no backend e persiste o usuário em armazenamento local.
 * @param {{ email: string, name: string, picture?: string }} userData
 * @returns {Promise<any>} Dados do usuário retornados pela API em caso de sucesso
 * @throws {Error} Erro com mensagem amigável conforme o tipo (403, 500, rede, genérico)
 */
export async function verifyAndSaveUser(userData) {
  ...
  try {
    ...
    // Tratamento por status code antes de tentar ler o body
    if (response.status === 403) {
      throw new Error('Acesso negado. Apenas emails @imrea.com são permitidos.');
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(serverMessage || 'Falha na verificação do usuário');
    }
    ...
    // Erros de rede (TypeError em fetch) ou outros
    ...
    // Propaga a mensagem que já mapeamos acima (403/500/genérico)
